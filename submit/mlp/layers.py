...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        conv_shape=((self.input_height-grads_wrt_outputs.shape[2]+1),(self.input_width-grads_wrt_outputs.shape[3]+1))
        temp_matrix=np.zeros(conv_shape)
        result_shape=(inputs.shape[0],inputs.shape[1]) + conv_shape
        result=np.zeros(result_shape)
